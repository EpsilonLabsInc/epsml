...
def assign_projections(args):
    ...
    # Load projections CSV file.
    print(f"Loading projections CSV file {args.projections_csv_path}")
    projections_df = pd.read_csv(args.projections_csv_path, low_memory=False, header=None)

    # Create projections dict.
    print("Creating projections dict")
    projections_dict = dict(zip(projections_df[0], projections_df[1]))

    # Assign projections.
    projections = []
    print("Assigning projections")
    for image_path in image_paths:
        if image_path is None:
            projections.append(None)
        else:
            values = [projections_dict[item] if item in projections_dict else None for item in image_path]
            projections.append(values)

    # Update projection classification column.
    print("Updating projection classification column")
    assert len(projections) == len(df)
    df["projection_classification"] = projections
...
if __name__ == "__main__":
    ...
    PROJECTIONS_CSV_PATH = "/home/andrej/tmp/gradient_batches_1-5_segmed_batches_1-4_simonmed_batches_1-10_reports_with_labels_all_projections.csv"
