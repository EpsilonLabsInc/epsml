...
class I3DResNet(torch.nn.Module):
    def __init__(self, resnet2d, frame_nb=16, class_nb=1000, conv_class=False):
        ...
        self.conv1 = inflate_utils.inflate_conv(
            resnet2d.conv1, time_dim=3, time_padding=1, center=True)
        self.bn1 = inflate_utils.inflate_batch_norm(resnet2d.bn1)
        self.relu = torch.nn.ReLU(inplace=True)
        self.maxpool = inflate_utils.inflate_pool(
        ...
        if conv_class:
            self.avgpool = inflate_utils.inflate_pool(resnet2d.avgpool, time_dim=1)
            ...
            self.avgpool = inflate_utils.inflate_pool(
                resnet2d.avgpool, time_dim=final_time_dim)
            self.fc = inflate_utils.inflate_linear(resnet2d.fc, 1)
...
class Bottleneck3d(torch.nn.Module):
    def __init__(self, bottleneck2d):
        ...
        self.bn1 = inflate_utils.inflate_batch_norm(bottleneck2d.bn1)

        self.conv2 = inflate_utils.inflate_conv(
            bottleneck2d.conv2,
            time_dim=3,
            time_padding=1,
            time_stride=spatial_stride,
            center=True)
        self.bn2 = inflate_utils.inflate_batch_norm(bottleneck2d.bn2)

        self.conv3 = inflate_utils.inflate_conv(
            bottleneck2d.conv3, time_dim=1, center=True)
        self.bn3 = inflate_utils.inflate_batch_norm(bottleneck2d.bn3)
...
def inflate_downsample(downsample2d, time_stride=1):
    downsample3d = torch.nn.Sequential(
        ...
        inflate_utils.inflate_batch_norm(downsample2d[1]))
    return downsample3d
